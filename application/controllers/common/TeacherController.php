...
class TeacherController extends CI_Controller
{
	...
	{
		// 1. Recibir los datos del POST
		$idasignacion = $this->input->post('idasignacion');
		$nombre = $this->input->post('nombre');
		$descripcion = $this->input->post('descripcion');
		$usuarios = $this->input->post('usuarios'); // Esto es un array de IDs

		// 2. Armar el array para la tabla principal (ejemplo: 'asignaciones')
		$data_asignacion = array(
			'nombre' => $nombre,
			'descripcion' => $descripcion,
			// 'updated_at' => date('Y-m-d H:i:s') // Si manejas fechas de actualización
		);

		// 3. Actualizar la tabla principal
		$this->db->where('id', $idasignacion);
		$this->db->update('asignaciones', $data_asignacion);

		// 4. Actualizar los estudiantes asignados
		// Lo más seguro y fácil en BD relacionales es borrar las asignaciones 
		// viejas de este ID y volver a insertar el array nuevo.
		$this->db->where('asignacion_id', $idasignacion);
		$this->db->delete('asignacion_usuario'); // Cambia por el nombre de tu tabla pivote
		...
		$this->db->insert_batch('asignacion_usuario', $batch_data);

		// 5. Responder a la petición AJAX
	...
	public function contenidoInicial()
	{
		...
		$path = FCPATH . 'application/uploads/images/';
		...
		if (! $this->upload->do_upload('image')) {
			...
			$this->Contenido->insertar_contenido_inicial([
				...
				'contenido'   => $contenido,
				'image'       => $imagen['file_name'],
				'rutaimg'     => 'http://127.0.0.1:8080/hipermedia/application/uploads/images/' . $imagen['file_name'],
				'seccion_id'  => $seccionid,
				'created_at'  => date('Y-m-d H:i:s')
			]);

			$this->session->set_flashdata('success', 'Nuevo contenido subido de manera exitosa');
			redirect('docente');
		}
	}
	...
	// Método para actualizar los datos editados
	public function editarContenidoInicial()
	{
		if ($this->session->has_userdata('email') && $this->session->userdata('tipo') == 'Docente') {
			...
			// Buscamos el registro actual en base de datos para recuperar los datos viejos de la imagen
			$contenidoActual = $this->Contenido->traer_contenido_id($idcontenido);
			...
			// Evaluamos si el usuario decidió subir una nueva imagen
			if (!empty($_FILES['image']['name'])) {
				...
				if (!$this->upload->do_upload('image')) {
					...
					$imagenNueva = $this->upload->data();

					// Borramos físicamente la imagen anterior si es que existe
					if (!empty($contenidoActual[0]->image)) {
						@unlink($path . $contenidoActual[0]->image);
					}

					// Actualizamos los campos de imagen con los nuevos valores del cargador
					$dataUpdate['image']   = $imagenNueva['file_name'];
					$dataUpdate['rutaimg'] = 'http://127.0.0.1:8080/hipermedia/application/uploads/images/' . $imagenNueva['file_name'];
				}
			}

			// Enviamos los datos recolectados al modelo. 
			// Nota: Asegúrate de que tu modelo 'Contenido' posea una función para realizar este update de manera estándar.
			$this->db->where('id', $idcontenido);
			$this->db->update('contenido_inicial', $dataUpdate);

			$this->session->set_flashdata('success', 'Contenido inicial modificado de manera exitosa.');
			redirect('docente');
		} else {
			redirect('');
		}
