...
class Contenido extends CI_Model {
	...
	public function actualizar_contenido($idtema, $datac, $datat)
	{
		// 1. Actualiza los datos del tema (lapso, descripción del tema, etc.)
		$this->db->where('id', $idtema);
		$this->db->update('temas', $datat);

		// 2. Actualiza el documento o texto largo en la tabla contenidos
		$this->db->where('tema_id', $idtema);
		$this->db->update('contenidos', $datac);
